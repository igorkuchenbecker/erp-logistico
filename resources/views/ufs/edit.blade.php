...
@section('content')
    ...
    <form action="{{ route('ufs.update', $uf) }}" method="POST"
        ...
        <div>
            ...
            <input type="text" name="codigo" value="{{ old('codigo', $uf->codigo) }}" required maxlength="50"
        ...
        <div class="flex gap-4">
            <div class="flex-1">
                ...
                <input type="number" step="0.01" min="0" name="peso"
                       value="{{ old('peso', $uf->peso) }}" required
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none">
            ...
            <div class="flex-1">
                ...
                <input type="text" name="tipo_item" value="{{ old('tipo_item', $uf->tipo_item) }}" required maxlength="100"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none">
        ...
        <div class="flex gap-4">
            <div class="flex-1">
                ...
                <input type="text" name="origem" value="{{ old('origem', $uf->origem) }}" required maxlength="150"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none">
                @error('origem') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">Destino</label>
                <input type="text" name="destino" value="{{ old('destino', $uf->destino) }}" required maxlength="150"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none">
