...
@section('content')
    ...
    <div class="grid grid-cols-3 md:grid-cols-5 gap-4 mb-8">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Pendentes</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-orange-600">{{ $porStatus['aguardando_coleta'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Aguar. Coleta</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-cyan-600">{{ $porStatus['coletado'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Coletados</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-purple-600">{{ $porStatus['unitizado'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Unitizados</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-teal-600">{{ $porStatus['liberado_programacao'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Lib. Programação</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-blue-600">{{ $porStatus['em_transito'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Em Trânsito</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-green-600">{{ $porStatus['entregue'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Entregues</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4 text-center">
            <p class="text-3xl font-bold text-red-600">{{ $porStatus['cancelado'] ?? 0 }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Cancelados</p>
        </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4">
            <h2 class="font-bold text-gray-700 dark:text-gray-200 mb-3">Top Destinos</h2>
            @if($porDestino->isEmpty())
                <p class="text-gray-400 dark:text-gray-500 text-sm">Nenhum dado ainda.</p>
            @else
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <tr>
                            <td class="py-2">
                                <a href="{{ route('destino.show', $item->destino) }}" class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">
                                    {{ $item->destino }}
                                </a>
                            </td>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4">
            <h2 class="font-bold text-gray-700 dark:text-gray-200 mb-3">Em Trânsito</h2>
            @if($emTransito->isEmpty())
                <p class="text-gray-400 dark:text-gray-500 text-sm">Nenhuma carga em trânsito.</p>
            @else
                <div class="space-y-3">
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                        <div class="flex items-center justify-between mb-1">
                            <a href="{{ route('rastreamento.show', $uf->codigo_rastreio) }}"
                               class="text-emerald-700 dark:text-emerald-400 font-bold hover:underline text-lg">
                                {{ $uf->codigo_rastreio }}
                            </a>
                        ...
                        @if($uf->prazo_entrega)
                            <p class="text-xs mt-1 {{ $uf->prazo_entrega->isPast() ? 'text-red-500' : 'text-gray-400 dark:text-gray-500' }}">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow p-4">
            <h2 class="font-bold text-gray-700 dark:text-gray-200 mb-3">Últimas UFs Cadastradas</h2>
            @if($recentes->isEmpty())
                <p class="text-gray-400 dark:text-gray-500 text-sm">Nenhuma UF cadastrada.</p>
            @else
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 dark:text-gray-400">
